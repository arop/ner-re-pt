...
do
	...
	do
		...
		../join-output-golden.sh $IN_NER/out-cat-iob.txt $IN_GOLD/t_cat_test-iob.txt | ../conlleval > $OUT_RES/cat.txt
	done

	python ../src/avg-results.py stanford-ner cat $r
done

python ../src/avg-results-all.py stanford-ner cat