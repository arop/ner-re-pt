...
if(len(sys.argv) > 4):
  ...
  print "Usage: python " + sys.argv[0] + " <tool> <level> <parameter> <value>\n"
  sys.exit()
...
results_files = []
for r in range(4):
	f = open('../results/' + tool + '/repeat-' + str(r) + '/experiences/' + parameter + '/' + value + '/' + level + '.txt', 'r')
...
f = open('../results/' + tool + '/avg/experiences/'+ parameter + '/' + value + '/' + level + '-avg.txt', 'w')
f.write(to_file)
f.close()
